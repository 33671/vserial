 ...
 * Requires: /dev/vserialctl and /dev/ttyGSV* devices
 */
...
int main() {
    ...
    for (int pair_idx = 0; pair_idx < info.num_pairs; pair_idx++) {
        ...
        printf("==================================================\n");
        system("ls -l /dev/ttyGSV0");
        // race conditions, wait for udev to finish
        sleep(1);
        system("ls -l /dev/ttyGSV0");
        // Open first port in the pair (A)
        snprintf(dev_path, sizeof(dev_path), "/dev/ttyGSV%d", info.pairs[pair_idx].minorA);
        ...
        snprintf(dev_path, sizeof(dev_path), "/dev/ttyGSV%d", info.pairs[pair_idx].minorB);
